...
const S={run:false,finished:false,speed:0,progress:0,lap:1,time:0,last:0,lapStart:0,best:Infinity};const key={left:false,right:false,gas:false,brake:false};
...
function reset(){S.run=false;S.finished=false;S.speed=0;S.progress=0;S.lap=1;S.time=0;S.last=0;S.lapStart=0;S.best=Infinity;player.position.set(0,.02,-R);player.rotation.y=Math.PI;cars.slice(1).forEach((c,i)=>{c.progress=(i+1)*.035;let a=-Math.PI/2+c.progress*Math.PI*2;c.mesh.position.set(Math.cos(a)*(R+c.lane*.12),.02,Math.sin(a)*(R+c.lane*.12))})}
async function start(){document.getElementById('start').classList.add('hidden');document.getElementById('finish').classList.add('hidden');reset();await countdown();S.run=true;S.lapStart=0}
...
let pos=1;cars.slice(1).forEach(c=>{if(c.progress>S.progress)pos++});document.getElementById('stats').textContent=`LAP ${Math.min(S.lap,3)}/3   POS ${Math.min(pos,6)}/6   ${fmt(S.time)}`;document.getElementById('speed').innerHTML=`${Math.round(S.speed*3.6)} <small>KM/H</small>`;S.time+=dt}
